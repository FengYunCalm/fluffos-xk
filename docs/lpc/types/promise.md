...
* Settling a promise returned by an `async` function is an error: it is the
  body's result channel, and the body settles it. Doing so would discard
  whatever the body returns without stopping it running. The refusal is
  limited to those: a promise from `promise_create()`, `promise_then()`, or
  the promise form of a callback efun stays settleable — settling one just
  means the driver's later result is dropped when it arrives.
