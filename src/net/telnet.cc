...
// Websocket connections use the same table, but refuse MCCP (COMPRESS2): the
// websocket layer has its own compression (permessage-deflate), so MCCP would
// only re-wrap the stream in a second deflate and hide the telnet framing from
// the ws message-boundary logic.
...
struct telnet_t *net_telnet_init(interactive_t *user) {
  // Self-compressing transports (websockets) get the table that refuses MCCP;
  // ip->lws is set before this runs on the websocket paths.
...
static inline void on_telnet_do(unsigned char cmd, interactive_t *ip) {
  switch (cmd) {
    ...
    case TELNET_TELOPT_COMPRESS2:
      // MCCP is refused on websocket transports (see my_telopts_ws); libtelnet
      // answers WONT for them and this event normally never fires, but guard
      // anyway so compression cannot start there.
      if (!ip->lws) {
    ...
    case TELNET_TELOPT_LINEMODE: {
      ...
      switch (action) {
        case LM_MODE:
          ...
          // #1247 TELNET-1: A LINEMODE MODE/DO/WILL sub-option carries a second
          // byte; guard it since only size >= 1 (buf[0]) is guaranteed above --
          // a client can send a 1-byte sub-negotiation and make buf[1] an
          // out-of-bounds read (reflected back to the client in DO/WILL cases).
          if (size < 2) {
            break;
          }
          if (!(buf[1] & MODE_ACK)) {
        ...
        /* refuse FORWARDMASK */
        case TELNET_DO: {
          // #1247 TELNET-1: guard buf[1] (see LM_MODE above).
          if (size < 2) {
            break;
          }
...
void on_telnet_do_zmp(const char **argv, unsigned long argc, interactive_t *ip) {
  ...
  // #1247 TELNET-3: destination index is i-1 -- filling item[1..argc-1] would
  // write one past the argc-1 element array and leave item[0] uninitialized.
