...
static promise_t* require_promise(svalue_t* value, const char* efun) {
  if (value == nullptr || value->type != T_PROMISE || value->u.prom == nullptr) {
    error("%s: expected a promise.\\n", efun);
  }
  return value->u.prom;
}

static funptr_t* require_handler(svalue_t* value, const char* efun) {
  if (value == nullptr || value->type != T_FUNCTION || value->u.fp == nullptr) {
    error("%s: expected a function handler.\\n", efun);
  }
  return value->u.fp;
}
...
void f_promise_resolve() {
  ...
  promise_t* p = require_promise(parg, "promise_resolve");
...
void f_promise_reject() {
  int const num_arg = st_num_arg;
  svalue_t* parg = sp - num_arg + 1;
  promise_t* p = require_promise(parg, "promise_reject");
...
static void promise_then_common(int num_arg, funptr_t* on_fulfilled, funptr_t* on_rejected) {
  svalue_t* parg = sp - num_arg + 1;
  promise_t* p = require_promise(parg, "promise_then");
...
void f_promise_then() {
  ...
  funptr_t* on_fulfilled =
      (num_arg > 1) ? require_handler(parg + 1, "promise_then") : nullptr;
  funptr_t* on_rejected =
      (num_arg > 2) ? require_handler(parg + 2, "promise_then") : nullptr;
...
void f_promise_catch() {
  svalue_t* parg = sp - 1;
  require_promise(parg, "promise_catch");
  promise_then_common(2, nullptr, require_handler(sp, "promise_catch"));
}
#endif

#ifdef F_PROMISE_STATUS
void f_promise_status() {
  promise_t* p = require_promise(sp, "promise_status");
...
void f_promise_result() {
  promise_t* p = require_promise(sp, "promise_result");
