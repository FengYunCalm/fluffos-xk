...
void f_clear_bit() {
  ...
  // Check the sign on the untruncated 64-bit value: a large-magnitude negative
  // LPC_INT can truncate into a positive `int` that would slip past a
  // post-truncation check, bypassing max_bitfield_bits entirely.
  if (sp->u.number < 0) {
...
void f_include_list() {
  array_t *vec = include_list(sp->u.ob);
      ...
      switch (find->type | (sv = v->item + tmp)->type) {
        case T_STRING:
          if (flag & 1) {
            ...
            if (strncmp(find->u.string, sv->u.string, flen) != 0) {
              continue;
            }
          } else {
            if (flen && (sv->subtype & STRING_COUNTED) && flen != MSTR_SIZE(sv->u.string)) {
            ...
            if (strcmp(find->u.string, sv->u.string) != 0) {
              continue;
            }
          }
          break;
...
#ifdef F_ORIGIN
void f_origin() { push_constant_string(origin_name(caller_type)); }
#endif
...
void f_query_load_average() { copy_and_push_string(query_load_av()); }
#endif
    ...
    if (plen > 1) { /* pattern length > 1, jump table most efficient */
      while (src < flimit) {
        if ((skip = skip_table[static_cast<unsigned char>(src[probe])])) {
          // #1247 EFUNS-1: this fast-path copy is not covered by the dlen-based
          // guards below; bound it against the output buffer and count it in
          // dlen so a later match's expansion guard sees the true written length.
...
void f_query_heart_beat() {
  int const num = query_heart_beat(sp->u.ob);
...
void f_strsrch() {
  ...
  size_t const src_len = SVALUE_STRLEN(arg1);
...
void f__this_object() { push_object(current_object); }
#endif
...
#ifdef F_UPTIME
void f_uptime() { push_number(get_current_time() - boot_time); }
#endif
...
void f_set_clean_up() {
  ...
  /* Same gate as at load/clone time: only objects that actually define
   * clean_up() are queried by the sweep. */
