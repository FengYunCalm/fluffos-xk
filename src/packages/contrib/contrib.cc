...
static void deep_copy_svalue(svalue_t *from, svalue_t *to) {
  switch (from->type) {
    ...
    case T_MAPPING: {
      ...
      // Copy the child FIRST, then write the destination slot in one go:
      // `to` points into a container that outlives an error() unwind (freed
      // by the RAII guards above), so it must never hold `from`'s pointer
      // without owning a reference while a nested copy can still throw.
      svalue_t nv = *from;
      switch (from->type) {
        case T_ARRAY:
