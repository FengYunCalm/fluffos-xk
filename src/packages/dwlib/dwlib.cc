...
svalue_t *replace_objects(svalue_t *thing) {
  ...
  switch (thing->type) {
    case T_OBJECT: {
      // obname and, especially, the master's object_name() apply below are
      // arbitrary-length and mudlib/master-controlled -- bound every copy into
      // this fixed buffer instead of strcpy/strcat.
      char buf[2000];
      size_t len = 0;
    ...
    if (oldstr[i] == '$') {
      ...
      for (j = 0; j < sp->u.arr->size; j += 2) {
        ...
        if (!strncmp(one, oldstr + i, COUNTED_STRLEN(one))) {
          ...
          // #1247 DWLIB-1: check against the replacement length actually
          // written below ('two'), not the matched pattern ('one'): a
          // replacement longer than its pattern would otherwise overrun newstr.
