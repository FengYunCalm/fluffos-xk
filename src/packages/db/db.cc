...
static array_t *MySQL_fetch(dbconn_t *c, int row) {
  ...
  // #1247 DB-1: per-field byte lengths of the current row; field->max_length
  // is the column-wide maximum across all rows, so using it to size/copy a
  // BLOB over-reads any row whose value is shorter than that maximum.
      ...
      switch (field->type) {
        ...
        case FIELD_TYPE_VAR_STRING:
          if (field->flags & BINARY_FLAG) {
            // #1247 DB-2: use this row's actual field length, not the
            // column-wide max.
...
static array_t *SQLite3_fetch(dbconn_t *c, int row) {
  ...
  if (last_row < row) {
     ...
     * after last_row instead of row N. */
