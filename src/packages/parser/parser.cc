...
static array_t *master_user_list = nullptr;
...
static parse_result_t *best_result = nullptr;
...
static saved_error_t *parallel_errors = nullptr;
...
/* UTF-8 lead/continuation bytes are word bytes: the C locale classifies them
 * as non-printable and non-alphanumeric, which used to silently strip every
 * non-ASCII character during tokenization, so non-ASCII words never reached
 * STR rules or OBJ id matching. Matching stays bytewise (no case folding
 * beyond ASCII). */
...
static void remove_ids(parse_info_t *pinfo) {
  ...
  if (pinfo->flags & PI_SETUP) {
    ...
    pinfo->ids = pinfo->plurals = pinfo->adjs = nullptr;
  }
}
...
static void hash_clean() {
  ...
  for (i = 0; i < HASH_SIZE; i++) {
    for (nodep = &hash_table[i]; *nodep && ((*nodep)->flags & HV_PERM); nodep = &((*nodep)->next)) {
      ;
    }
    while (*nodep) {
      next = (*nodep)->next;
    ...
    switch (check_special_word(str, &tmp)) {
      ...
      case SW_ALL:
        ...
        if (tok & PLURAL_MODIFIER) {
          ...
          mp = add_match(&local_state, ERROR_TOKEN, start, state->word_index - 1);
    ...
    while (hnode) {
      ...
      if (hnode->name == str) {
        ...
        if (singular_legal && (hnode->flags & HV_NOUN)) {
          ...
          mp = add_match(&local_state, tok & ~PLURAL_MODIFIER, start, state->word_index - 1);
          bitvec_copy(&mp->val.obs, &objects);
          if (ordinal != 0) {
            mp->ordinal = ordinal;
          } else {
      ...
      default:
        if (tok <= 0) {
          ...
          *p++ = ' ';
          ...
          if (cnt == which - 1 || ++ocnt >= which || (matches[cnt].token & PLURAL_MODIFIER)) {
            p = strput(p, end, "that ");
          } else {
            /* FIXME */
 ...
 */
static int parallel_process_answer(parse_state_t *state, svalue_t *sv, int which) {
  if (!sv) {
    return 0;
  }
  if (sv->type == T_NUMBER) {
  ...
  while ((tok = parse_vn->token[index++])) {
    *buf++ = '_';
    switch (tok & ~CHOOSE_MODIFIER) {
      ...
      case OBS_TOKEN:
      case ADD_MOD(OBS_TOKEN, VIS_ONLY_MODIFIER):
        if (omatch + 1 >= which || !(matches[match].token & PLURAL_MODIFIER) || (which >= 4)) {
          buf = strput(buf, end, "obj");
        } else {
...
static void singular_check_functions(int which, parse_state_t *state, match_t *m) {
...
static int use_cached_parallel_error(parse_state_t *state, parser_error_t *err) {
  if (!err->error_type) {
    return 0;
  }
  if (state->num_errors++ == 0) {
...
static void dependent_check_functions(int which, parse_state_t *state, match_t *m) {
  bitvec_t *bv = &m->val.obs;
  int i, k;
  unsigned int j;
  int found_one = 0;
  ...
  if (!found_one &&
      (use_cached_parallel_error(state, &errinfo) || use_last_parallel_error(state))) {
    m->token = ERROR_TOKEN;
  }
  /* Translate me!
 ...
 * further processing).
 */
static int check_one_relation(parse_state_t *state, int direct_first, parser_error_t *errinfo) {
  int res;
  int ob;
  parser_error_t err;
  ...
  res = parallel_check_functions(loaded_objects[direct_first ? direct_object : indirect_object],
                                 state, direct_first ? 4 : 5);
  if (!res) {
  ...
  res = parallel_check_functions(loaded_objects[direct_first ? indirect_object : direct_object],
                                 state, direct_first ? 5 : 4);
  if (!res) {
...
static void we_are_finished(parse_state_t *state) {
  ...
  if (state->num_errors) {
    ...
    DEBUG_P(
        ("Saving successful match: %s (%s)", best_result->res[0].func, best_result->ob->obname));
  }
  DEBUG_DEC;
}
...
static void parse_rules() {
  ...
  while (parse_vn) {
    ...
    if ((!parse_restricted || parse_vn->handler == parse_restricted) &&
      ...
      pos = 0;
      if ((parse_vn->lit[0] == -1 || (pos = check_literal(parse_vn->lit[0], 1))) &&
...
static void parse_recurse(char **iwords, char **ostart, char **oend) {
  ...
  // with a hard budget: once tripped, every pending parse_recurse() frame
  // returns immediately, running its normal cleanup on the way out (a plain
  // return chain, not an error() that would skip it). (Upstream 8b0aee8a
  // shares parse_rule's counter; the local parser has no such counter, so a
  // dedicated budget is used instead.)
  static int parse_recurse_steps = 0;
  constexpr int kMaxParseRecurseSteps = 1000000;
  if (++parse_recurse_steps > kMaxParseRecurseSteps) {
    return;
  }
...
void f_parse_my_rules() {
  ...
  if (best_match) {
    if (flag) {
      ...
      best_result->res[3].args = nullptr;
      sp--; /* pop the error handler */
      free_string_svalue(sp--);
      free_svalue(sp, "parse_my_rules"); /* may have been destructed */
      put_array(arr);
...
void f_parse_add_synonym() {
  ...
  if (new_verb) {
    ...
    while (verb_entry) {
      if (verb_entry->real_name == new_verb && verb_entry->match_name == old_verb) {
        if (rule) {
