...
*/
// The vector helpers used to signal errors by returning special sentinel
// float values (-INT_MAX, -INT_MAX+1, -INT_MAX+2) instead of a real result. A
// legitimate result can coincide with one of them (e.g. a dotprod() of
// large-magnitude vectors landing on -INT_MAX+1), misreporting a valid
// computation as an error -- and the error() calls formatted that LPC_FLOAT
// sentinel with "%d" (a printf format/argument mismatch). Report
// success/failure out of band instead.
enum { VEC_OK = 0, VEC_ERR_SIZE_MISMATCH = -1, VEC_ERR_ARG1 = 1, VEC_ERR_ARG2 = 2 };

static LPC_FLOAT norm(array_t *a, bool *ok) {
  // Length must come from the array we actually index (a): f_angle() calls
  // norm((sp-1)->u.arr), a different array, so using sp's size would read past
  // `a` if the two ever differed in length.
  LPC_INT len = a->size;
  LPC_FLOAT total = 0.0;
  *ok = true;
