...
static int restore_internal_size(const char **str, int is_mapping, int depth, int nest) {
  ...
  // #1247 OBJ-2: bound *recursion* depth the same way the save path does
  // (too_deep_save_error), so a maliciously deep-nested save string (e.g.
  // "({({({...") cannot recurse until the C stack overflows. This must track
  // true nesting depth (`nest`), NOT `depth`: `depth` is save_svalue_depth, a
  // monotonic index into sizes[] that counts every container across the whole
  // structure in DFS order -- bounding `depth` wrongly rejected valid
  // wide-but-shallow data (e.g. a mapping of a few hundred one-element
  // arrays) once the cumulative container count crossed the cap. Returning 0
  // fails the size pre-pass so restore_size() reports an error before the
  // actual restore recurses.
...
static int restore_class(char **str, svalue_t *ret) {
  ...
  if (size < 0 || size > CONFIG_INT(__MAX_ARRAY_SIZE__)) {
    // Same reasoning as restore_array()'s guard: allocate_class_by_size()
  ...
  // Owned via RAII -- same reasoning as restore_array()'s v_owned: a
