...
parse_node_t *new_node() {
  ...
  if ((cur_block = free_block_list)) {
    ...
    cur_block = reinterpret_cast<parse_node_block_t *>(
        DMALLOC(sizeof(parse_node_block_t), TAG_COMPILER, "new_node"));
    ...
    if (r->kind == NODE_NUMBER) {
      switch (op) {
        ...
        case F_LSH:
          ...
          // undefined behavior; mask to the low 6 bits (mod 64), matching the
          // runtime opcode (f_lsh()/f_rsh() in ops.cc) so a folded and
        ...
        case F_MOD:
          ...
          // #1247 TREES-1: a % -1 == 0; computing it directly traps (SIGFPE)
          // for INT_MIN.
          if (r->v.number == -1) {
            l->v.number = 0;
            break;
