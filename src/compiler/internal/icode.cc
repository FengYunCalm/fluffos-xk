...
static void switch_to_line(unsigned int line) {
...
// thousands of '+' terms) builds a parse tree of matching depth with no bound
// from the grammar or parser -- walking it here recurses just as deep. Cap it
// so pathological input errors cleanly instead of overflowing the C stack.
// NODE_TWO_VALUES chains (top-level definitions, statement lists) are walked
// iteratively below instead of recursed, so they never count against this:
// the cap only ever bounds genuine expression nesting.
  ...
  switch (expr->kind) {
    ...
    case NODE_BINARY_OP:
      ...
      if (expr->v.number == F_ASSIGN || expr->v.number == F_VOID_ASSIGN) {
        ...
        if (dest && IS_NODE(dest, NODE_OPCODE_1, F_LOCAL_LVALUE)) {
          i_generate_node(expr->l.expr);
          end_pushes();
        ...
        if (dest && IS_NODE(dest, NODE_OPCODE_1, F_GLOBAL_LVALUE)) {
          i_generate_node(expr->l.expr);
          end_pushes();
    ...
    case NODE_TWO_VALUES: {
      // rule_program_append() chains one node per top-level definition via
      // l.expr; the statements: rule chains one per statement via r.expr.
      // Neither is safe to recurse: l.expr isn't a tail call (r.expr's own
      // work follows it), and r.expr only looks tail-callable -- the depth
      // guard's DEFER above still has to run after it returns. Flatten with
      // an explicit stack instead, in whichever direction the chain nests;
      // i_generate_node() is still called (and still depth-capped) per leaf,
      // so a genuinely deep expression inside one definition/statement is
      // still caught.
    ...
    case NODE_SWITCH_RANGES: {
      ...
      if (expr->v.expr && expr->v.expr->kind == NODE_DEFAULT) {
        upd_short(addr + 5, expr->v.expr->v.number - addr, "switch");
    ...
    case NODE_FUNCTION_CONSTRUCTOR:
      ...
      switch (expr->v.number & 0xff) {
        ...
        case FP_FUNCTIONAL | FP_NOT_BINDABLE: {
          ...
          end_pushes();
          ins_byte(F_RETURN);
          upd_short(addr, CURRENT_PROGRAM_SIZE - addr - 2, "function pointer");
          break;
        }
      }
      break;
...
void i_update_forward_branch(const char *what) {
  end_pushes();
  nforward_branches--;
  upd_short(forward_branches[nforward_branches],
            CURRENT_PROGRAM_SIZE - forward_branches[nforward_branches], what);
}
